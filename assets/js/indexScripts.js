...
const main = async function () {
    ...
    const datosPersona = async function () {
    ...
    const actualizarListaPersonasHTML = async function () {
    ...
    await actualizarListaPersonasHTML();

    listaPersonasHTML.addEventListener("click", async function (event) {
        if (event.target.tagName === "BUTTON") {
            idSeleccionado = event.target.id;
            await datosPersona();
            await calcularFfmiBrmMc();
            await actualizarListaPersonasHTML();
        }
    });
